        ...
        if frame is None:
            ...
            if face_detection_net.requests[cur_request_id].wait(-1) == 0:
                ...
                for face_loc in face_detection_res[0][0]:
                    if face_loc[2] > 0.5:
                        xmin = int(face_loc[3] * initial_w)
                        ymin = int(face_loc[4] * initial_h)
                        xmax = int(face_loc[5] * initial_w)
                        ymax = int(face_loc[6] * initial_h)

                        # ------ age - gender
                        age_gender_net, a_n, a_c, a_h, a_w, a_input_blob, a_out_blob = load_xml_bin('include/age-gender-recognition-retail-0013.xml', 'include/age-gender-recognition-retail-0013.bin')
                        age_gender_frame = frame[ymin:ymax, xmin:xmax]
                        ...
                        if age_gender_net.requests[cur_request_id].wait(-1) == 0:
                            ...
                            cv2.putText(frame, gender + ', ' + str(age), (ymax, ymin), cv2.FONT_HERSHEY_DUPLEX, 1.0, (255, 255, 255), 1)

                        # ------ emotion
                        emotion_net, e_n, e_c, e_h, e_w, e_input_blob, e_out_blob = load_xml_bin('include/emotions-recognition-retail-0003.xml', 'include/emotions-recognition-retail-0003.bin')
                        ...
                        if emotion_net.requests[cur_request_id].wait(-1) == 0:
                            ...
                            cv2.putText(frame, emotion, (ymax, ymin + 20), cv2.FONT_HERSHEY_DUPLEX, 1.0,
                                        (255, 255, 255), 1)
